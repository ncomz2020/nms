...
<script>
	var strDeptCd = ${project[0].dept_cd};
	var init_yn = "Y";
	
	$(document).ready(function(){
		console.log(strDeptCd);
		console.log("strDeptCd");
		var num = 1;
		var textData = [];
	...
	function memberList(){		
		var List = '<c:forEach items="${member}" var="memberInfo" varStatus="status">';
	...
	function changeCmbM(cmb){
		...
		$.ajax({
			...
			success: function(data){
				...
				var teamHtml = '<option value=""><spring:message code="label.common.select" /></option>';
				for(var i=0; i<teamList.length; i++){
					teamHtml += '<option value="' + teamList[i].dtl_cd +'">' + teamList[i].dtl_cd_desc + '</option>';
				}
				$("#team_cd").html(teamHtml);
				if(init_yn == "Y"){
					var strTeamCd = ${project[0].team_cd}+''; // null 값의 경우 null 값이 아닌 ''로  표현하도록 세팅
					if(strTeamCd == ''){
	...
	function deletePj(){
		...
		if( sessionId != '1') {
			swal({
				title: '<spring:message code="label.project.delete.error" />',
...
<div class="rowBox mgT20">
	<div class="g_column w_1_1">
		<h3>
			...
			<input type="hidden" id="pre_pjt_no" name="pre_pjt_no" value="${project[0].pjt_no }"/>
				<table class="amb_form_table">
					...
					<tbody>
						<tr>
							...
							<td>
							<c:choose>
								<c:when test="${project[0].check_yn == 2}">
						...
						<tr>
							...
							<td>
							<c:choose>
								<c:when test="${project[0].pjt_type_cd == 'N11'}">
								...
								<c:when test="${project[0].pjt_type_cd == 'N12'}">
									<select id="pjt_type_cd" name="pjt_type_cd" class="inp w90p">
							...
							<td>
								<input type="text" id="pjt_no" name="pjt_no" class="inp _input w90p" value="${project[0].pjt_no }"  maxlength="9"/>
							</td>
							<th><spring:message code="label.project.pjtname" /></th>
							<td>
								<input type="text" id="pjt_name" name="pjt_name" class="inp _input w90p" value="${project[0].pjt_name}" />
							</td>
						</tr>
						...
						<tr>
							...
							<td>
								<input type="text" id="cust_nm" name="cust_nm" class="inp _input w90p" value="${project[0].cust_nm}"/>
		                    </td>
		                    
		                    <th><spring:message code="label.project.pmnm" /></th>
		                    <td>
		                    	<input type="text" id="pm_nm" name="pm_nm" class="inp _input w90p" value="${project[0].pm_nm}"/>
		                    </td>
						</tr>
						...
						<tr>
							...
							<td>
							<c:choose>
								<c:when test="${project[0].cont_yn == 2}" >
							...
							<td>
							<c:choose>
								<c:when test="${project[0].use_yn == 2}">
							...
							<td>
								...
								<input type="text" id="strt_dt" name="strt_dt" class="inp datepicker startDate" onkeydown="onlyNumber(this)" value="${project[0].strt_dt}"/>
								~
								<input type="text" id="end_dt" name="end_dt" class="inp datepicker endDate" onkeydown="onlyNumber(this)" value="${project[0].end_dt}"/>
								</span>
							</td>
						</tr>
						...
						<tr>
							...
							<td>
							<c:choose>
								<c:when test="${project[0].etc_yn == 2}">
